...
- **Verify before committing:** `dotnet format Upkeep.sln --verify-no-changes`, `dotnet build Upkeep.sln --configuration Release`, `dotnet test Upkeep.sln --configuration Release`.
...
- **Built so far:** repository scaffolding, governance docs and ADRs. Implementation in progress — update this section as real pieces land, the way Pulsemap's own CLAUDE.md tracks them.
