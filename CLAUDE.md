...
- **Built so far:**
  - **Shell:** DI in `App.xaml.cs`, `NavigationView` rail with all eight sections, Mica title bar, Home as a dashboard (health, next action, recent sessions, drives), a Ctrl+K command palette, the three-screen welcome tour shown once on first launch, and a toast when a cleanup finishes while the user is on another page. Pages inside the frame ask the shell for window-level things through `IMessenger` (`ShellMessages.cs`). Language is settled in `Program.Main` *before* `Application.Start` — XAML resolves `x:Uid` against the resource context in place when a page first loads, so it can't be changed later in the run; Settings says "after a restart" for that reason.
  ...
  - **Cleanup (complete):** `JunkCatalog` (12 categories, the rules in one table), `JunkScanner` (user scope) and `SystemJunkScanner` (machine scope, helper-side), `CleanupPlan` → `CleanupExecutor`, `SystemJunkCleaner`. The twelfth is the user's own rules (`CustomCleanupRule`): the one quarantined category, since those files are the user's rather than a cache Upkeep derived, and the only one whose scope is validated rather than known — a rule can cover the user's profile or another drive, never Windows, another account, Upkeep's own data, or a whole drive unfiltered. Windows' own tooling does the irreversible parts: DISM for the component store (never `/ResetBase`), the Delivery Optimization cmdlet for its cache, `SHEmptyRecycleBin` for the bin.
  ...
  - **History & Settings:** every session listed, one-click revert of a whole session, and the settings page — which also replays the welcome tour and shows the tail of the diagnostic log in place, so reading it doesn't mean a trip to Explorer.
  - **Tests:** 559 Core, 224 App; Core branch coverage 86.0% against the 80% floor.
